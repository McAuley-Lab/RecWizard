...
class ReDialDataCollatorForRec:
    ...
    def __call__(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, Union[torch.Tensor, Any]]:
        """
...
class RecommenderTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        for batch in tqdm(dataloader):
            input, target = batch["input"], batch["target"]
            input = {
                k: v.to(device) if type(v) is torch.Tensor else [x.to(device) for x in v] for k, v in input.items()
            }
        ...
        metrics = {f"recall@{n}": np.mean(recalls[n]) for n in ["1", "10", "50"]}
...
if __name__ == "__main__":
    ...
    rec_module.load_state_dict(
        torch.load(args.autorec_model),
        strict=True,
        allow_unexpected=True,
        LOAD_IGNORES=("sentiment_analysis",),
        LOAD_MAPPINGS={"": "recommender."},
    )
    rec_module.load_state_dict(
        torch.load(args.sa_model),
        strict=True,
        allow_unexpected=True,
        LOAD_IGNORES=("recommender",),
        LOAD_MAPPINGS={"": "sentiment_analysis."},
    )
