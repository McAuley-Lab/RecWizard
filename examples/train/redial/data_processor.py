...
class RedialDataProcessor:
    ...
    def __init__(self, tokenizer=None, tokenize_for_rnn=False, dataset="redial"):
        ...
        if self.tokenize_for_rnn:
            self.vocab = load_json_file_from_dataset(dataset, "vocab.json")
    ...
    def map_redial_for_recommender(self, example):
        ...
        movie_occurrences = [
            self._fill_movie_occurrences(encoding, texts, movie_name=self.id2name[id]) for id in movieIds
        ]
        dialogue = []
        target = []
        raw_texts = []
