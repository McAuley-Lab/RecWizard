...
from recwizard.utility import batchify, init_deterministic
...
from recwizard.utility import STATE_DICT_FILE, DeviceManager
...
class ConvTrainer(Trainer):
    def __init__(self, gen_dataset, tokenizer: BaseTokenizer, gen_file=None, debug=False, **kwargs):
        ...
        self.tokenizer = tokenizer
        self.evaluator = ConvEvaluator(tokenizer=self.tokenizer,
                                       log_file_path=os.path.join('unicrs/log', f'{gen_file}.jsonl'))
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default='save/unicrs_conv/redial', help="Where to store the final model.")
...
if __name__ == '__main__':
    ...
    data_collator = CRSDataCollatorForConv(
        tokenizer=tokenizer,
        use_amp=False, debug=args.debug,
    )
    trainer = ConvTrainer(
        model=model,
