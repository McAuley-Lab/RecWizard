...
class KGDataLoader:
    ...
    def get_entity_kg_info(cls, dataset):
        ...
        with open(
            os.path.join(dataset_dir, "dbpedia_subkg.json"), "r", encoding="utf-8"
        ) as f:  # extracted in extract_subkg
            entity_kg = json.load(f)
        with open(os.path.join(dataset_dir, "entity2id.json"), "r", encoding="utf-8") as f:  # only need len
            entity2id = json.load(f)
        with open(
            os.path.join(dataset_dir, "relation2id.json"), "r", encoding="utf-8"
        ) as f:  # just use len(relation2id) = len(relation_set)+1
        ...
        if cls.debug:
            logging.debug(
                f"#edge: {len(edge)}, #relation: {num_relations}, " f"#entity: {num_entities}, #item: {len(item_ids)}"
            )
        return {
...
class UnicrsDataProcessor:
    ...
    def __init__(
        ...
        super().__init__()
        self.debug = debug
        entityName2id = load_json_file_from_dataset(dataset, "entityName2id.json")
    ...
    def prepare_data_for_pretrain(self, batched_dialog):
        ...
        for dialog in batched_dialog.pa_table.to_pylist():
            ...
            entities = [
                self.entity2id[m.group(1)] for m in self.entity_pattern.finditer(resp) if m.group(1) in self.entity2id
            ]
    ...
    def prepare_data_for_conv(self, dialog, gen=False):
        ...
        return {"context": context, "resp": resp}
    ...
    def merge_gen_data(self, dialog, idx, gen_data):
        ...
        if self.default_movie_entity in pred:
            message = (
                message[:resp_start] + " " + pred
            )  # The space was stripped by the evaluator, we need to add it back
        else:
            message = message[:resp_start]
        return {"messages": message, "rec": dialog["rec"]}
