...
from recwizard.utils import STATE_DICT_FILE, DeviceManager, init_deterministic
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="save/unicrs_rec/redial", help="Where to store the final model."
    )
    ...
    parser.add_argument(
        "--gen_data",
        type=str,
        default="save/unicrs_conv/redial/{}_gen.jsonl",
...
if __name__ == "__main__":
    ...
    for subset in datasets:
        ...
        datasets[subset] = datasets[subset].map(
            dp.prepare_data_for_rec,
            batched=True,
            with_indices=True,
            fn_kwargs={"gen_data": gen_data},
            remove_columns=["recNames"],
            load_from_cache_file=False,
        )

    num_update_steps_per_epoch = math.ceil(
        len(datasets["train"]) / args.per_device_train_batch_size / args.gradient_accumulation_steps
    )
