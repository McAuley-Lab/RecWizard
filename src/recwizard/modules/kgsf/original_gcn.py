""" This file is adapted from the KGSF original implementation: https://github.com/Lancelot39/KGSF/blob/master/models/graph.py
"""
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        e_rowsum = self.special_spmm(edge, edge_e, torch.Size([N, N]), torch.ones(size=(N, 1)).to(device))
        # e_rowsum: N x 1
...
class SpGAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads):
        ...
        self.dropout = dropout
        self.out_att = SpGraphAttentionLayer(nhid, nclass, dropout=dropout, alpha=alpha, concat=False)
