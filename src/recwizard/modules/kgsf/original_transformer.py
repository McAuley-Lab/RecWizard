...
""" This file is adapted from the KGSF original implementation: https://github.com/Lancelot39/KGSF/blob/master/models/transformer.py
"""
...
from recwizard.modules.kgsf.original_utils import neginf
...
def create_position_codes(n_pos, dim, out):
    ...
    out_temp = out.clone()
    out_temp[:, 0::2] = torch.FloatTensor(np.sin(position_enc)).type_as(out)
    out_temp[:, 1::2] = torch.FloatTensor(np.cos(position_enc)).type_as(out)
    out = out_temp.clone()
    out.detach_()
...
class TransformerEncoder4kg(nn.Module):
    ...
    def __init__(
        ...
        assert embedding_size % n_heads == 0, "Transformer embedding size must be a multiple of n_heads"
...
class TorchGeneratorModel(nn.Module):
    ...
    def reorder_encoder_states(self, encoder_states, indices):
        ...
        :rtype:
            model specific
        """
        raise NotImplementedError("reorder_encoder_states must be implemented by the model")
    ...
    def reorder_decoder_incremental_state(self, incremental_state, inds):
        ...
        :rtype:
            model specific
        """
        raise NotImplementedError("reorder_decoder_incremental_state must be implemented by model")
    ...
    def forward(self, *xs, ys=None, cand_params=None, prev_enc=None, maxlen=None, bsz=None):
        ...
        if ys is not None:
            ...
            scores, preds = self.decode_greedy(encoder_states, bsz, maxlen or self.longest_label)
